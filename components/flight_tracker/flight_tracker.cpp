...
namespace esphome
{
  namespace flight_tracker
  {
    ...
    {
      this->cancel_interval("check_stale_aircraft");
      this->close(true);
    }
    ...
    void HOT FlightTracker::draw_schedule()
    {
      ...
      // Sort aircraft by altitude descending
      std::sort(this->schedule_state_.aircraft.begin(), this->schedule_state_.aircraft.end(),
      ...
      size_t num_to_display = std::min((size_t)this->limit_, this->schedule_state_.aircraft.size());
      for (size_t i = 0; i < num_to_display; ++i)
      {
        const Aircraft &aircraft = this->schedule_state_.aircraft[i];
